...
CONF_ACTIONS = {
    CONF_EDIT_DEVICE: "更改包含的设备",
    CONF_CHANGE_TOKEN: "修改绑定token",
}
...
DEFAULT_DOMAINS = ['light', 'switch', 'cover', 'scene', 'climate', 'script']
...
async def _async_name_to_type_map(hass: HomeAssistant) -> dict[str, str]:
    ...
    # 使用中文映射替代原来的集成名称
    return {
        domain: DOMAIN_TRANSLATIONS.get(domain, domain)
        ...
        if user_input is not None:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    # 注意：公共配置端点不需要认证头
                    async with session.get(f"{base_url}/api/plugin/public_config", ssl=False) as response:
                        if response.status != 200:
                            _LOGGER.error(f"无法获取配置: {response.status}")
                            return self.async_show_form(
                                step_id="user",
                ...
                _LOGGER.debug(f"尝试获取版本信息从: {base_url}/api/plugin/version")
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(f"{base_url}/api/plugin/version", ssl=False) as response:
                            _LOGGER.debug(f"版本响应状态码: {response.status}")
                            if response.status == 200:
                                _LOGGER.debug(f"版本内容: {await response.text()}")
                            else:
                                _LOGGER.error(f"获取版本失败: {response.status}")
                except Exception as e:
                    _LOGGER.warning(f"版本检查失败，但将继续配置: {e}")

            except Exception as e:
                _LOGGER.error(f"验证失败: {str(e)}")
                return self.async_show_form(
                    step_id="user",
                    data_schema=vol.Schema({vol.Required(CONF_TOKEN): str}),
                    errors={"base": "unknown"},
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(f"{base_url}/api/plugin/basic_version", ssl=False) as response:
                        _LOGGER.debug(f"基本版本端点状态: {response.status}")
                        if response.status == 200:
                            version_data = await response.json()
                            _LOGGER.debug(f"基本版本信息: {version_data}")
            except Exception as e:
                _LOGGER.warning(f"版本检查失败，但将继续配置: {e}")
                # 继续执行，不返回错误
